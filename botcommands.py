...
async def help_disp(bot=None, message=None, admin=False, args=None):
    await bot.send_message(message.channel, help_text(message.author))


async def whomai(bot=None, message=None, admin=False, args=None):
    await bot.send_message(message.channel, message.author.id)


async def echo(bot=None, message=None, admin=False, args=None):
    try:
...
async def roll(bot=None, message=None, admin=False, args=None):
    try:
        n = int(args[1])
...
async def flip(bot=None, message=None, admin=False, args=None):
    await bot.send_message(message.channel, random.choice(['Heads', 'Tails']))


async def eightball(bot=None, message=None, admin=False, args=None):
...
async def command(bot, message, admin):
    ...
    if command_info is not None:
        await command_info.func(bot=bot, message=message, admin=admin, args=args)
